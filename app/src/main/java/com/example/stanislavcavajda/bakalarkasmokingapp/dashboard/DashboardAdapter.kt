...
class DashboardAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        when(viewType) {
            R.layout.main_progress -> {
                var mainProgressBinding = com.example.stanislavcavajda.bakalarkasmokingapp.databinding.MainProgressBinding.inflate(layoutInflater!!)
                return MainProgressViewHolder(mainProgressBinding)
            }
            R.layout.healthprogress_dashboard_item_new -> {
                var healthProgressBinding = HealthprogressDashboardItemNewBinding.inflate(layoutInflater!!)
                return HealthProgressViewHolder(healthProgressBinding)
            }
            R.layout.wish_manager_dashboard_item -> {
                var wishBinding = WishManagerDashboardItemBinding.inflate(layoutInflater!!)
                return WishViewHolder(wishBinding)
            }
            R.layout.money_saved_dashboard_item -> {
                var moneySavedBinding = MoneySavedDashboardItemBinding.inflate(layoutInflater!!)
                return MoneyViewHolder(moneySavedBinding)
            }
            else -> {
                var mainProgressBinding = MainProgressBinding.inflate(layoutInflater!!)
                return MainProgressViewHolder(mainProgressBinding)
            }
        }
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is MainProgressViewHolder) {
            this.layoutInflater?.let {
                MainProgressBinding.inflate(it).viewModel = this.list[0] as MainProgressViewModel
            }
